...
public class MultiLineChartActivity extends DemoBase implements OnSeekBarChangeListener,
    ...
    //我定义的格式类
    class MyselfValueFormatter extends ValueFormatter{
        String[] weeks = {"期一","期二","期三","期四","期五","期六","期日"};
        @Override
        public String getAxisLabel(float value, AxisBase axis) {
            int index = new Float(value).intValue();
            if (index<7 && index>=0)
                return weeks[index]+" "+super.getAxisLabel(value,axis);
            else
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        chart.getAxisLeft().setEnabled(false);//左轴总开关，禁用
        chart.getAxisRight().setDrawAxisLine(false);
        chart.getAxisRight().setDrawLabels(false);//我加的，不标刻度文字
        //chart.getAxisRight().setDrawGridLines(false);//我注释的，要画刻度线
        chart.getXAxis().setDrawAxisLine(false);
       chart.getXAxis().setDrawGridLines(false);
       //下面是我加的
        chart.getXAxis().setLabelCount(7,true);
        chart.getXAxis().setValueFormatter(new MyselfValueFormatter());
...
//我加的代码，固定设置为7，一星期7天，配合修改x周的lable用，产生7个数据
        progress = 7;
        ...
        ((LineDataSet) dataSets.get(0)).setColors(Color.RED);//ColorTemplate.VORDIPLOM_COLORS);//我改的颜色
