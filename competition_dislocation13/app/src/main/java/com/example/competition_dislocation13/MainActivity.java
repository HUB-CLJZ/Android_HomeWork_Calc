package com.example.competition_dislocation13;
...
public class MainActivity extends AppCompatActivity {
    ...
    private TextView tvLightNumber;
    ...
    //显示当前参数值的方法
    private void showListeningValue() {
        tvHumidityNumber.setText("当前湿度：" + String.format("%02d", pbHumidity.getProgress()));
        if (pbHumidity.getProgress() > maxValueHumidity) {
            tvHumidityNumber.setTextColor(Color.RED);
        } else {
            tvHumidityNumber.setTextColor(Color.GREEN);
        }

        tvLightNumber.setText("当前光照：" + String.format("%02d", pbLight.getProgress()));
        if (pbLight.getProgress() > maxValueLight) {
            tvLightNumber.setTextColor(Color.RED);
        } else {
            tvLightNumber.setTextColor(Color.GREEN);
        }

        tvTemperatureNumber.setText("当前温度：" + String.format("%02d", pbTemperature.getProgress()));
        if (pbTemperature.getProgress() > maxValueTemperature) {
            tvTemperatureNumber.setTextColor(Color.RED);
        } else {
            tvTemperatureNumber.setTextColor(Color.GREEN);
        }
    }
    ...
    private void initView() {
        ...
        tvTemperatureNumber = findViewById(R.id.tv_temperature_number);
        tvHumidityNumber = findViewById(R.id.tv_humidity_number);
        tvLightNumber = findViewById(R.id.tv_light_number);
    }
